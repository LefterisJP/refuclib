...
int main()
{
	...
	rfString_PruneEnd(&s,1);//since the low order digit is rounded in an implementation defined manner we just drop it
	...
	EXPECTNOT(RF_FAILURE,rfStringX_MoveAfter(&s,RFS_("-"),&result,0)) //the exponent should be minus
	EXPECT(true,rfString_Equal(&s,RFS_("6 \n"))) //the exponent should be -6
	EXPECT(RF_SUCCESS,rfStringX_Format(&s,"Small num %.12E \n",0.000003234))
	rfPrintf("Negative %.2e\n",-567.2);
	
	
	//utf8 characters, cstrings and Refu strings
