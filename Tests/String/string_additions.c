...
int main()
{
    ...
    EXPECT(rfString_Append(&s3,RFS_(" %s is %s and its value is %.4f","number","the golden mean",1.6180339)), true)
    rfPrintf("%S\n",&s3);
	...
	EXPECT(rfStringX_Append(&sx1,RFS_("研究室に行ってきます。")), true)
	EXPECT(true,rfString_Equal(&sx1,RFS_("the train from Kita Senju to Kashiwa station.研究室に行ってきます。")))
	EXPECT(rfStringX_Prepend(&sx2,RFS_("オバマ氏再選　ロムニー氏を振り切る　米大統領選 || ")), true)
	...
	EXPECT(rfStringX_Insert(&sx2,17,RFS_("|| Δεύτερη θητεία στην προεδρία των ΗΠΑ κατέκτησε ο Μπαράκ Ομπάμα ")), true)
