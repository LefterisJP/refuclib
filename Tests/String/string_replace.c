...
int main()
{
    ...
    EXPECT(true,rfString_Init(&s1,"gcc -c -I$INCLUDES $FLAGS $FILE.c -o ./$OBJDIRNAME/$FILE.o"))
    EXPECT(true,rfString_Replace(&s1,RFS_("$INCLUDES"),RFS_("../../includes"),0,0))
    ...
    EXPECT(true,rfString_Replace(&s2,RFS_("中国メディア"),RFS_("Chinese media"),0,0))
    EXPECT(true,rfString_Replace(&s2,RFS_("９月"),RFS_("September"),0,0))
    EXPECT(true,rfString_Replace(&s2,RFS_("就役した"),RFS_("commisioned"),0,0))
    EXPECT(true,rfString_Replace(&s2,RFS_("初の空母"),RFS_(" new Carrier"),0,0))
    EXPECT(true,rfString_Replace(&s2,RFS_("搭載する"),RFS_("load"),0,0))
    EXPECT(true,rfString_Replace(&s2,RFS_("輸出用戦闘機"),RFS_("Fighter used for exporting"),0,0))
    EXPECT(true,rfString_Equal(&s2,RFS_("Chinese mediaはＪ３１について、Septemberにcommisioned new Carrier「遼寧」号などにload新型艦載機やFighter used for exportingになるとの見方を伝えている。")))

    //expect the same String only with many english words replaced with Greek equivalents and check for equality
    EXPECT(true,rfString_Init(&s3,"Journalist Costas Vaxevanis has gone on trial in Athens for breach of privacy after publishing the names of 2,000 Greeks with Swiss bank accounts"))
    EXPECT(true,rfString_Replace(&s3,RFS_("Journalist"),RFS_("Ο Δημοσιογραφος"),0,0))
    EXPECT(true,rfString_Replace(&s3,RFS_("for breach of privacy"),RFS_("για καταπατηση προσωπικων δικαιωματων"),0,0))
    EXPECT(true,rfString_Replace(&s3,RFS_("after publishing"),RFS_("αφου δημοσιευσε"),0,0))
    EXPECT(true,rfString_Replace(&s3,RFS_("the names"),RFS_("τα ονοματα"),0,0))
    EXPECT(true,rfString_Replace(&s3,RFS_("with Swiss bank accounts"),RFS_("με λογαριασμους σε Ελβετικες τραπεζες"),0,0))
