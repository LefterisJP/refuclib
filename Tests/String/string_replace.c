...
int main()
{
    ...
    EXPECT(true,rf_string_replace(&s1,RFS_("$FLAGS"),RFS_("-Wall -O2"),0,0));
    EXPECT(true,rf_string_replace(&s1,RFS_("$FILE"),RFS_("tocompile"),0,0));
    EXPECT(true,rf_string_replace(&s1,RFS_("$OBJDIRNAME"),RFS_("obj"),0,0));
    ...
    EXPECT(true,rf_string_replace(&s2,RFS_("９月"),RFS_("September"),0,0));
    EXPECT(true,
           rf_string_replace(&s2,RFS_("就役した"),RFS_("commisioned"),0,0));
    EXPECT(true,
           rf_string_replace(&s2,RFS_("初の空母"),RFS_(" new Carrier"),0,0));
    EXPECT(true,
           rf_string_replace(&s2,RFS_("搭載する"),RFS_("load"),0,0));
    EXPECT(true,
           rf_string_replace(&s2,
                            RFS_("輸出用戦闘機"),
    ...
    EXPECT(true,
           rf_string_replace(&s3,RFS_("the names"),RFS_("τα ονοματα"),0,0));
    EXPECT(true,
           rf_string_replace(
               &s3,
	...
	//Stringx replacing
    EXPECT(true,rf_stringx_init(&sx1,"News from all over the world."));
    EXPECT_NOT(RF_FAILURE,rf_stringx_move_after(&sx1,RFS_("News "), 0, 0));
    EXPECT(true,
           rf_stringx_replace(
               &sx1,
               RFS_("."),
