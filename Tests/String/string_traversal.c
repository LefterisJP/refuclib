...
int main()
{
	...
	EXPECTNOT(RF_FAILURE,rfStringX_MoveAfter(&sx1,RFS_("　"),&resx1,RF_STRINGX_ARGUMENT))
	...
	EXPECT(true,rfString_Equal(&sx2,RFS_(" περίπου μία ώρα αργότερα, κορυφώνονται το Σάββατο οι εκδηλώσεις για την 39η επέτειο από την Εξέγερση του Πολυτεχνείου. Λόγω των εκδηλώσεων, θα ισχύσουν έκτακτες κυκλοφοριακές ρυθμίσεις έως και το πρωί της Κυριακής.")))
	EXPECT(true,rfString_Equal(&res2,RFS_("και την πορεία προς την αμερικανική πρεσβεία")))
	
	EXPECT(true,rfStringX_Init(&sx3,"これがMoveAfterPairのテストですね！「Δεν θα επιστραφω εγω」　Γεμιζουμε το στρινγκ με διαφορα 「ブラケットの中のテキストは結果になる」Let's see if the function will work as expected."))	
	EXPECT(true,rfStringX_MoveAfterPair(&sx3,RFS_("「"),RFS_("」"),&res3,0,2))
	EXPECT(true,rfString_Equal(&sx3,RFS_("Let's see if the function will work as expected.")))
	EXPECT(true,rfString_Equal(&res3,RFS_("ブラケットの中のテキストは結果になる")))
