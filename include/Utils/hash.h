 ...
 * what the fastest hash to use is.
 *
 * See also: rf_hash_str_64, rf_hash_str_stable.
 */
#define rf_hash_str(str, base) hash_any(rf_sting_data(str),         \
                                        rf_string_lengt_bytes(str), \
                                        (base))
...
/**
 * rf_hash_str_stable - hash of an RFstring for external use
 * @str: The RFstring to hash on
 * @base: the base number to roll into the hash (usually 0)
 *
 * This hash will have the same results on different machines, so can
 * be used for external hashes (ie. hashes sent across the network or
 ...
 *
 */
#define rf_hash_str_stable(str, base)                                   \
    (BUILD_ASSERT_OR_ZERO(sizeof(char) == 8 || sizeof(char) == 4        \
                          || sizeof(char) == 2 || sizeof(char) == 1) +  \
     sizeof(char) == 8 ? hash_stable_64(rf_string_data(str), rf_string_length_bytes(str), (base)) \
     : sizeof(char) == 4 ? hash_stable_32(rf_string_data(str), rf_string_length_bytes(str), (base)) \
     : sizeof(char) == 2 ? hash_stable_16(rf_string_data(str), rf_string_length_bytes(str), (base)) \
     : hash_stable_8(rf_string_data(str), rf_string_length_bytes(str), (base)))
