        ...
        if self.has_template != "":
            ...
            if arg_env[self.has_template] != []:
                ...
                for d_type in arg_env[self.has_template]:
                    ...
                    generated_sources = []
                    for _, structure in (
                            template.structures[d_type].iteritems()):
                    ...
                    # if not it's the user's responsibility to copy the
                    # generated .c file into his project
...
def create_includes(template, root):
    ...
    f = open(
        os.path.join(
            root, "include", template.parent_dir, template.name + "_extra.h"),
        "w")
...
modules.append(
    Module("HASHMAP",
           ...
           template_files={
               "shallow": "Data_Structures/hashmap_shallow.ctemplate",
